...
# =========================================================
# 사용자 API 기반 사이트 확인 (served_by 필드 사용)
# =========================================================

DR_PROBE_URL="${DR_PROBE_URL:-https://ccmall.shop/api/v1/weather?api_key=${DR_PROBE_API_KEY:-}&region=11&start_date=2026-01-01&end_date=2026-01-02}"
...
check_external_site() {
  ...
  for ((attempt = 1; attempt <= HEALTH_RETRY; attempt++)); do
    ...
    served_site="$(
      printf '%s\n' "${body%$'\n'*}" |
        grep -o '"served_by"[[:space:]]*:[[:space:]]*"[^"]*"' |
        sed -E 's/.*:[[:space:]]*"([^"]*)"/\1/'
    )"

    if [[ "$http_code" == "200" && "$served_site" == "$expected_site" ]]; then
      log INFO \
        "사용자 API 확인 성공 expected=${expected_site} actual=${served_site}"
      return 0
    fi

    log WARN \
      "사용자 API 대기 expected=${expected_site} code=${http_code:-000} actual=${served_site:-unknown} attempt=${attempt}/${HEALTH_RETRY}"

    sleep "$HEALTH_INTERVAL"
  done

  log ERROR \
    "사용자 API 확인 실패 expected=${expected_site}"
...
TIMELINE_FILE="${LOG_DIR}/dr-timeline.csv"
TIMELINE_LOCK_FILE="${LOG_DIR}/dr-timeline.lock"

if [[ ! -f "$TIMELINE_FILE" ]]; then
  printf '%s\n' \
    'timestamp_utc,epoch_ms,run_id,scenario,marker,source,target,message' \
    > "$TIMELINE_FILE"
fi
...
record_marker() {
  ...
  log INFO \
    "DR_MARKER marker=${marker} source=${source} target=${target} message=${message}"

  if command -v flock >/dev/null 2>&1; then
    (
      flock -x 9
      printf '%s\n' "$record" >> "$TIMELINE_FILE"
    ) 9>>"$TIMELINE_LOCK_FILE"
  else
    printf '%s\n' "$record" >> "$TIMELINE_FILE"
  fi
}
...
get_marker_epoch_ms() {
  ...
  awk -F',' \
    -v run_id="$RUN_ID" \
    -v marker="$marker" \
    '$3 == run_id && $5 == marker { value=$2 } END { print value }' \
    "$TIMELINE_FILE"
}
...
record_duration_between() {
  ...
  start_ms="$(get_marker_epoch_ms "$start_marker")"
  end_ms="$(get_marker_epoch_ms "$end_marker")"

  if [[ -z "$start_ms" || -z "$end_ms" ]]; then
    log WARN \
      "DR_METRIC_SKIP metric=${metric} start=${start_marker} end=${end_marker}"
    return 0
  fi
  ...
  record_step \
    ...
    "timeline" \
    "SUCCESS" \
    "$(awk "BEGIN { printf \"%.3f\", ${duration_ms}/1000 }")" \
    "${start_marker}->${end_marker}; duration_ms=${duration_ms}"

  log INFO \
    "DR_METRIC metric=${metric} duration_ms=${duration_ms}"
}